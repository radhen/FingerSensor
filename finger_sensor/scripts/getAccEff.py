...
class FilterValues(object):
	...
	def filter(self):
		...
		np.savetxt(path+'/SAI_Fgr.txt', self.Fgr) #can use this to plot in matlab graspeventdetection_plot.m
		np.savetxt(path+'/SAI_Fgl.txt', self.Fgl) #can use this to plot in matlab
		...
		np.savetxt(path+'/FAII.txt', self.FAII) #can use this to plot in matlab
		...
		np.savetxt(path+'/FAI.txt', self.FAI) #can use this to plot in matlab
	...
	def plot(self):
		...
		xgripper = np.linspace(0,(self.gripperAperture.shape[0]/20),self.gripperAperture.shape[0]) #getiing the time scale correct.
